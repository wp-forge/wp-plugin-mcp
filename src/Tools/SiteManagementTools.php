...
trait SiteManagementTools {
	...
	private function add_site_abilities() {
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'user_search', 'Search Users', 'Search and filter WordPress users with pagination', $users_search_schema, function ( $params ) {
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'user_get', 'Get User', 'Get a WordPress user by ID', $user_get_schema, function ( $params ) {
			return $this->get_user( (int) $params['id'] );
		}, true, 'list_users' );
		$this->add_ability( self::INTERNAL_PREFIX . 'user_save', 'Save User', 'Create or update a WordPress user', $user_write_schema, function ( $params ) {
			return $this->save_user( $params );
		}, false, 'edit_users' );
		$this->add_ability( self::INTERNAL_PREFIX . 'user_delete', 'Delete User', 'Delete a WordPress user by ID', $user_get_schema, function ( $params ) {
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'general_settings_get', 'Get General Settings', 'Get WordPress general site settings', $this->schema(), function () {
			return $this->get_general_settings();
		}, true, 'manage_options' );
		$this->add_ability( self::INTERNAL_PREFIX . 'general_settings_save', 'Save General Settings', 'Save WordPress general site settings', $settings_schema, function ( $params ) {
