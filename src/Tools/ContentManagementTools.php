...
trait ContentManagementTools {
	...
	private function add_content_abilities() {
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'post_type_list', 'List Post Types', 'List registered WordPress post types with runtime validation metadata', $list_post_types_schema, function ( $params ) {
			return $this->list_post_types( $params );
		} );
		$this->add_ability( self::INTERNAL_PREFIX . 'content_search', 'Search Content', 'Search and filter content for any registered post type', $search_content_schema, function ( $params ) {
			return $this->search_content( $params );
		} );
		$this->add_ability( self::INTERNAL_PREFIX . 'content_get', 'Get Content', 'Get a content item from any registered post type by ID or slug', $content_id_schema, function ( $params ) {
			return $this->get_content( $params );
		} );
		$this->add_ability( self::INTERNAL_PREFIX . 'content_save', 'Save Content', 'Create or update content for any registered post type', $save_content_schema, function ( $params ) {
			return $this->save_content( $params );
		}, false );
		$this->add_ability( self::INTERNAL_PREFIX . 'content_delete', 'Delete Content', 'Delete content from any registered post type', $delete_content_schema, function ( $params ) {
