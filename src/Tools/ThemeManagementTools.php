...
trait ThemeManagementTools {
	...
	private function add_theme_abilities() {
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'theme_list', 'List Themes', 'List installed WordPress themes and their activation state', $this->schema(), function () {
			return $this->list_themes();
		}, true, 'switch_themes' );

		$this->add_ability( self::INTERNAL_PREFIX . 'theme_install', 'Install Theme', 'Install a WordPress theme from the WordPress.org theme directory by slug', $this->schema(
			array(
		...
		$this->add_ability( self::INTERNAL_PREFIX . 'theme_activate', 'Activate Theme', 'Activate an installed WordPress theme by stylesheet directory name', $stylesheet_schema, function ( $params ) {
			return $this->activate_theme_tool( $params['stylesheet'] );
		}, false, 'switch_themes' );

		$this->add_ability( self::INTERNAL_PREFIX . 'theme_delete', 'Delete Theme', 'Delete an installed WordPress theme by stylesheet directory name', $stylesheet_schema, function ( $params ) {
